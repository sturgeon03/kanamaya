...
class stanley_controller :
    def __init__(self):
        ...
        self.k = 1.3  # gain값
        self.cte_list = np.array([])  # CTE 값을 저장할 Numpy 배열
        self.idx_list = np.array([])  # closest_idx 값을 저장할 Numpy 배열
    ...
    def control(self):   
        while not rospy.is_shutdown():
            if self.is_path ==True and self.is_odom==True  :
                ...
                cte *= crosstrack_sign
                cte=-cte
                self.cte_list = np.append(self.cte_list, cte)
                self.idx_list = np.append(self.idx_list, closest_idx)
    ...
    def draw(self):
            plt.plot(self.idx_list, self.cte_list, 'ro-')  # CTE와 closest_idx를 사용하여 그래프 그리기
            plt.title('CTE vs Closest Index')
            plt.ylabel('Closest Index')
            plt.xlabel('CTE')
